...
- **Awards per Division** (decided; `competition/worldAwards.ts`): each Division has its own Most Valuable Player, Top Scorer (the most points in total, like soccer's top scorer), and All-Division Team (one team, its team of the season), using ZenGM's awards by division, since a World's divs mirror its Divisions. They replace the league-wide MVP and All-League teams, and a World has no Finals or Semifinals MVP, since it has no playoffs. The other ZenGM awards stay league-wide. Division MVPs don't act as MVPs, so the Hall of Fame and records pages don't count one for every Division. A new World gets these awards, and an existing World switches once when it loads (`worldAwardsPerDivision`, a game attribute) if its award settings are still the defaults. The Season Summary shows each Division's awards with the Division, and players' award lists name the Division by its initials, like other division awards.
