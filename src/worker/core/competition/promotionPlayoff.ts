 ...
 *
 * `playGame` plays one game in a round (the first is 0) and returns the
 * winner's tid. It's passed in so the bracket stays independent of any sport's
 * game simulation.
 */
const runPromotionPlayoff = async (
	...
	for (let round = 0; remaining.length > numSpots; round++) {
		...
		for (let i = 0; i < numGames; i++) {
			...
			const winner = await playGame(homeTid, awayTid, round);
